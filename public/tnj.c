...
static struct shellcode* get_defender_shellcode(struct shellcode *attacker) {
	...
	// close(1);
	// close(2);
	// INTENDED BUG: 0 1 2 is not closed, can be used to screw
	// attackers IO, which is a quick way to defend.
	// However, attacker can manually connect to the server,
	// detect such incorrect IO, and modify its IO output.
        close(code_pipe[0]);
